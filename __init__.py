...
def set_picture(image_file,_flag_rmbg):
	...
	_t_content = _buff_glob_rag_coll_x["content"] + "*0x00*: + _buff_yolo_arr_l
	_buff_glob_rag_coll_x.update("content":_t_content)
...
def get_inri_corpus_text_lib(_file_name):
	for _buff in _buff_glob_rag_coll_txt_att:
		_buff_k = _buff.get("file":_file_name)
...
def search_image(key_words, _buff_glob_lib_flag, pos_x, pos_y):
	...
	_buff_w = set_picture(_buff_blog_tx)
